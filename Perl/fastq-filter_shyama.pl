...
open BR1, " > $ARGV[0].bad.fastq" or die $!;
open BR2, " > $ARGV[1].bad.fastq" or die $!;

open GR1, " > $ARGV[0].good.fastq" or die $!;
open GR2, " > $ARGV[1].good.fastq" or die $!;
...
while(++$i) {
	my ($f1, $read1,FQ1) = get_next_read($f1, FQ1);
	warn "\n Returned next id is pair 1:$f1";
	my ($f2, $read2,FQ2) = get_next_read($f2, FQ2);
