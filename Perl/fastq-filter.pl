...
open BR1, "| gzip -c > $ARGV[0].bad.fastq.gz" or die $!;
open BR2, "| gzip -c > $ARGV[1].bad.fastq.gz" or die $!;

open GR1, "| gzip -c > $ARGV[0].good.fastq.gz" or die $1;
open GR2, "| gzip -c > $ARGV[1].good.fastq.gz" or die $1;
...
while(++$i) {
	my ($f1, $read1) = get_next_read($f1, FQ1);
	my ($f2, $read2) = get_next_read($f2, FQ2);
